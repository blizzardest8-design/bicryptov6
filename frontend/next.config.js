...
const nextConfig = {
  ...
  async rewrites() {
    // Always proxy API/uploads/static asset traffic from the frontend port
    // to the backend port on localhost. This is correct in BOTH dev and prod
    // when the frontend and backend run as two processes on the same host
    // (the standard Bicrypto deploy: PM2 with backend on 4000 + frontend on $PORT).
    //
    // Setting BACKEND_INTERNAL_URL lets you point at a different host (e.g. if
    // you split frontend and backend into separate Railway services), but the
    // default — same-host PM2 — works without any extra config.
    const backendUrl =
  ...
  allowedDevOrigins: [
    ...
    '*.ngrok-free.dev',
    '*.ngrok.io',
    // Replit dev tunnels (preview iframe)
    '*.replit.dev',
    '*.repl.co',
    '*.riker.replit.dev',
    '*.picard.replit.dev',
