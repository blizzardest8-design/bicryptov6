...
CREATE TABLE `admin_profit` (
  ...
  `description` text COMMENT 'Additional description of the profit source',
...
CREATE TABLE `category` (
  ...
  `image` text COMMENT 'URL path to the category''s featured image',
  `description` text COMMENT 'Description of the blog category',
  `createdAt` datetime NOT NULL,
  `updatedAt` datetime NOT NULL,
  `deletedAt` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `default_pages` (
  ...
  `content` text COMMENT 'HTML/markdown content for legal pages',
...
CREATE TABLE `exchange` (
  ...
  `description` text COMMENT 'Exchange provider description',
...
CREATE TABLE `exchange_market` (
  ...
  `isTrending` tinyint(1) DEFAULT 0 COMMENT 'Whether this market is currently trending',
  `isHot` tinyint(1) DEFAULT 0 COMMENT 'Whether this market is marked as hot/popular',
  `metadata` text COMMENT 'Additional market configuration and precision settings',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT 'Market availability status (active/inactive)',
  `currency` varchar(191) NOT NULL COMMENT 'Base currency symbol (e.g., BTC, ETH)',
  PRIMARY KEY (`id`),
...
CREATE TABLE `extension` (
  ...
  `description` text COMMENT 'Description of the extension functionality',
...
CREATE TABLE `ico_token_vesting_release` (
  ...
  `failureReason` text COMMENT 'Reason for failure if status is FAILED',
...
CREATE TABLE `kyc_application` (
  ...
  `adminNotes` text COMMENT 'Notes added by admin during KYC review process',
...
CREATE TABLE `kyc_level` (
  ...
  `description` text COMMENT 'Detailed description of the KYC level requirements',
...
CREATE TABLE `page` (
  ...
  `content` longtext NOT NULL COMMENT 'Main content/body of the page (HTML or Markdown)',
  `description` text COMMENT 'Brief description of the page content',
  `image` text COMMENT 'URL path to the page''s featured image',
  ...
  `seoDescription` text COMMENT 'SEO meta description for search engine results',
  `seoKeywords` text COMMENT 'SEO keywords for search engine optimization',
  `ogImage` text COMMENT 'Open Graph image URL for social media sharing',
  `ogTitle` varchar(255) DEFAULT NULL COMMENT 'Open Graph title for social media sharing',
  `ogDescription` text COMMENT 'Open Graph description for social media sharing',
  `settings` longtext COMMENT 'JSON string containing page-level configuration settings',
  `customCss` longtext COMMENT 'Custom CSS styles specific to this page',
  `customJs` longtext COMMENT 'Custom JavaScript code specific to this page',
...
CREATE TABLE `post` (
  ...
  `description` longtext COMMENT 'Brief description or excerpt of the post',
  `status` enum('PUBLISHED','DRAFT') NOT NULL DEFAULT 'DRAFT' COMMENT 'Publication status of the post (PUBLISHED or DRAFT)',
  `image` text COMMENT 'URL path to the featured image for the post',
  `createdAt` datetime NOT NULL,
  `updatedAt` datetime NOT NULL,
  `deletedAt` datetime DEFAULT NULL,
...
CREATE TABLE `settings` (
  `key` varchar(255) NOT NULL,
  `value` longtext COMMENT 'Setting value in JSON format or plain text',
  PRIMARY KEY (`key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `transaction` (
  ...
  `metadata` text COMMENT 'Additional transaction data in JSON format',
